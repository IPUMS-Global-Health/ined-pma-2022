...
## Mapping births by EA {auto-animate="true" visibility="uncounted"}
...
::: {.fragment}
Remember: our GPS data contain discrete points. 
:::
::: {.fragment}
```{r, echo = F} 
ea_summary_gps
...
:::
::: {.fragment}
We should use all of the available climate data for the complete **buffer zones**, which may span multiple pixels in our raster!
:::
...
:::: {.column width="60%"}
::::: {.fragment}
First we need to change the `geometry` of our data from arc-degrees to linear meters.
...
:::::
::::: {.fragment}
<span style='color: #98579B'>Failure to do so would mean that the length of 2000 or 5000 meters would differ as you travel away from the equator. </span>
:::::
::::
:::: {.column width=40%}
![](images/projection.gif){.absolute top=100 right=0 height=500}
::::
:::
...
::: {.fragment}
We'll use [EPSG code 32630](https://epsg.io/32630) to focus our new projection around Burkina Faso.
:::
::: {.fragment}
```{r}
...
::: {.fragment}
Draw a 2000 meter urban buffer, or a 5000 meter rural buffer around each centroid.
::: 
::: {.fragment}
```{r, results='hide'}
...
::::
:::: {.fragment}
We should only keep the segment containing the original centroid.
...
:::: {.fragment}
Finally, we can return to our original CRS. 
:::: 
:::: {.fragment}
```{r}
...
::: {.columns}
:::: {.column width=60%"}
Our example EA overlaps 6 CHIRPS pixels

```{r, results='hide'}
test_buffer <- ea_summary_buffer %>% 
  filter(ea == test_ea)
```
```{r, eval = FALSE}
ggplot() + 
...
:::: {.column width=60%"}
::::: {.fragment}
We want a spatial average of these 6 pixels, where the "weight" is the proportion of the pixel included in the buffer zone.
:::::
::::: {.fragment}
```{r}
